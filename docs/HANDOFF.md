...
T38 fixes Docker setup recovery and app updates during signup. `make start`
...
offering a refresh. Setup tokens remain in tab memory.
...
an isolated browser verified setup and the update guard. The maintainer chooses
their own credentials. No live account was created or reset by the agent.

Repository checks passed: **194 backend unit tests**, **145 frontend component
tests**, **254 integration tests**, and the disposable container setup check.
The full **62-case** desktop/mobile browser run preceded the signup update fix;
all **eight affected cases** passed afterward. Exact commands and changes are
in [TASKS](TASKS.md).

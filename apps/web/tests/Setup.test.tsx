...
describe("browser first-account setup", () => {
  ...
  it("shows a fixed server mismatch error without discarding a retryable setup link", async () => {
    install({
      failure: {
  ...
  it("does not keep authority across a reload, and does not put it back through Back navigation", async () => {
    install();
    const first = mount();
    await fill();
    fireEvent.click(screen.getByRole("link", { name: "Help" }));
